...
由第\ref{sec:state_of_the_art}节我们知道，目前正式投入使用的训练系统中的人体解剖模型（主要是血管系统模型）都是产品的固有部分，致使使用者无法及时地接触更多更富有实际意义的病例来进行学习与训练。
...
此外，系统还提供开放的数据输入功能，使所有基于真实的临床数据血管系统模型都能够即时读入。
...
触觉接口装置则是整个系统中除计算机本身以外唯一的硬件。
该装置实际上是一个机电一体化系统，用于为训练系统的使用者提供操作媒介。
它主要由提供模拟进给操作功能的机械部分，以及产生阻力的电机控制系统组成。
...
本文用到的主要工具有ITK\footnote{详见：\url{http://www.itk.org}}，VTK\footnote{详见：\url{http://www.vtk.org}}，VMTK\footnote{详见：\url{http://www.vmtk.org}}，IGSTK\footnote{详见：\url{http://www.igstk.org}}等；而在虚拟手术工具的建模和外观渲染方面的工作中，我们主要利用了Bullet\footnote{详见：\url{http://www.bulletphysics.org}}和OpenSceneGraph\footnote{详见：\url{http://www.openscenegraph.org}}。
目前，我们的训练系统样机运行于Windows操作系统上\footnote{由于在研究与开发的过程中，我们所采用的软件工具和开发方式，该系统样机也可以方便地移植到Linux操作系统上运行。}。
\begin{figure}[ht]
\centering
...
\end{itemize}
\item 血管系统模型的数据精简优化几何信息提取
\end{itemize}
人体心血管系统模型的重建是指从CTA中提取各解剖结构的对应信息并将其边缘表面进行可视化。
面对虚拟工具与血管模型交互过程中，系统对计算资源占用巨大，甚至导致仿真无法正常进行的实际情况，对血管系统模型进行合理的数据优化与精简是使系统保持性能的重要一环。
