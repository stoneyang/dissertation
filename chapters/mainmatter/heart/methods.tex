...
本章所述的基于CT的心脏分割工作的完整流程如图\ref{fig:heart_data_flow}所示。
首先，对原始影像数据进行平滑处理。
...
\caption[心脏区域分割的工作流程]{Flowchart of the segmentation work flow.}
...
\end{cases}
\end{equation}
where $\text{TH}_{\text{lower}}$ and $\text{TH}_{\text{upper}}$ mark the lower and upper thresholds of the intensity values, $I_{\text{input}}$ and $I_{\text{output}}$ mark the input and output image intensities.
\begin{figure}[t]
\centering
\input{FigureSrc/heart/binary_threshold}
\caption[二值阈值函数]{Binary thresholding function.}
...
\caption[用于非线性像素映射的S函数]{S-shaped function for nonlinear intensity mapping.}
...
If the speed function $F$ only depends the position of the point, then (\ref{eqn:heart_Eikonal}) reduces to the Eikonal equation.
...
In (\ref{eqn:heart_min_model}), $g(\cdot)$ is equivalent to the speed of the evolution of the curve.
...
To find the minimum of (\ref{eqn:heart_min_model}), the geodesic flow is calculated by
\begin{equation}
\label{eqn:heart_evolution_model}
...
Equation (\ref{eqn:heart_evolution_model}) shows the way $\mathcal{C}$ propagating towards the edges of the targets, i.e., the initial curve $\mathcal{C}(0)$ evolves to a (local) minimum of (\ref{eqn:heart_min_model}).

According to (\ref{eqn:heart_evolution_model}), this evolution will stop at edges of the targets when $\mathcal{C}_t = 0$.
%According to (\ref{eqn:heart_evolution_model}), this evolution will stop when $\frac{\partial \mathcal{C}(t)}{\partial t} = 0$, meaning that the boundaries of the objects are detected.
Imagine that $\mathcal{C}$ is a level-set of a function $u(\cdot):[0,a] \times [0,b] \rightarrow \mathrm{R}$, and considering (\ref{eqn:heart_min_model}), the former evolution (\ref{eqn:heart_evolution_model}) is transformed to finding the steady state solution of the following equation: %
\begin{equation}
\label{eqn:heart_level_set_model}
...
To increase the ``attraction" to the contour towards the edges, a term is added to (\ref{eqn:heart_level_set_model}):
\begin{equation}
\label{eqn:heart_geodesic_driver}
%\label{eqn:Geodesic1}
u_t = g( I_{\sigma} )( c + \kappa ) |\nabla u| + \nabla u \cdot \nabla g(I_{\sigma}),
%u_t = |\nabla u| \textrm{div} \left(g(I_{\sigma}) \frac{\nabla u}{|\nabla u|}\right) + c g(I_{\sigma}) |\nabla u|,
\end{equation}
where $c$ is a constant and $c \in \mathrm{R}^+$.
Regarding (\ref{eqn:heart_level_set_model}), %the above equation can be rewritten as
%\begin{equation}
%\label{eqn:Geodesic2}
%u_t = g( I_{\sigma} )( c + \kappa ) |\nabla u| + \nabla u \cdot \nabla g(I_{\sigma}).
%\end{equation}
% which means that the level sets move according to
equation (\ref{eqn:heart_geodesic_driver}) can be transformed to the following evolution equation
\begin{equation}
\label{eqn:heart_geodesic_evolution}
\mathcal{C}_t = g(I_{\sigma}) ( c + \kappa ) \mathcal{N} - ( \nabla g(I_{\sigma}) \cdot \mathcal{N} ) \mathcal{N}.
\end{equation}
The solution to this problem is equivalent to the zero level-set of the steady state (i.e., $u_t = 0$) of (\ref{eqn:heart_geodesic_driver}).
