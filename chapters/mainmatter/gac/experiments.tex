...
\fbox{\includegraphics[width=1.2in]{Figures/gac/dcm_threshold.eps}}%
\label{fig:aorta_threholding}}
\hfil
\subfloat[]{
\setlength{\fboxrule}{0.5pt}
\setlength{\fboxsep}{0cm}
\fbox{\includegraphics[width=1.2in]{Figures/gac/dcm_fastMarching.eps}}%
\label{fig:aorta_Fast_Marching}}
\hfil
\subfloat[]{
\setlength{\fboxrule}{0.5pt}
\setlength{\fboxsep}{0cm}
\fbox{\includegraphics[width=1.2in]{Figures/gac/dcm_geodesic.eps}}%
\label{fig:aorta_geodesic_active_contours}}
\hfil
\subfloat[]{
\setlength{\fboxrule}{0.5pt}
\setlength{\fboxsep}{0cm}
\fbox{\includegraphics[width=1.2in]{Figures/gac/dcm_out.eps}}%
\label{fig:aorta_binary_thresholding}}
\caption[水平集演进过程]{水平集演进过程：(a) 阈值滤波（$\text{TH}_{\text{lower}} = 300\text{HU}$, $\text{TH}_{\text{upper}} = 2000\text{HU}$）；(b) 从某一种子点开始，利用快速行进法进行的初始水平集演进；(c) 利用测地活动轮廓进行边缘检测（迭代次数: $100$）；(d) 利用二值阈值滤波完成像素亮度的反转（明亮区域的亮度值: $255$, 黑暗区域的亮度值: $0$）。}%
\label{fig:aorta_level_set_evolution}
\end{figure*}
...
对分割结果准确性的评估一直是医学影像领域的一个难题，到目前为止还没有一个在领域内获得公认的标准方法\cite{Nie2010}。
目前，分割结果准确性评估的方式有很多\cite{Zijdenbos1994}。
而在本章中，我们所提出的方法的准确性将通过Jaccard指数\cite{Jaccard1912}和Dice指数\cite{Zijdenbos1994}进行评估。
这两个指数分别通过不同方式表征分割结果与原始图像中的目标之间的相似度，而该相似度的大小则表示分割结果的准确性。
...
该指数值越大，则表明目标图像与源图像的交集占两者的并集的比重越大，也就意味着有更多的源图像中的区域被分割出来，从而说明分割结果的准确性越高。
...
\end{equation}
它又称为“均值叠加”，指两区域相交部分除以此两区域的平均面积。
与前一种指数的意义类似，该指数的值越大，则表明目标图像与源图像的交集占两者的平均面积的比重越大，也就意味着有更多的源图像中的区域被分割出来，从而说明分割结果的准确性越高。

表\ref{tbl:aorta_evaluation}记录本章实验中的十一组数据集两项指数的评估结果。
图\ref{fig:aorta_evaluation}显示指数评估结果的柱状图。
其中，这十一组数据集的Jaccard指数的平均值为$0.933$，标准差为$0.0240$；Dice指数的平均值为$0.965$，标准差为$0.0128$。
而根据这两个指数的定义，本章围绕GAC算法所设计的处理流程表现出了较高的准确性。
...
\subfloat[]{\includegraphics[width=3.0in,height=1.98in]{Figures/gac/Dice.eps}%
\label{fig:aorta_Dice}}
\caption[主动脉内腔分割结果评估]{主动脉内腔分割结果评估: (a) Jaccard系数；(b) Dice系数。}%
\label{fig:aorta_evaluation}
\end{figure}
