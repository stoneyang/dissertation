...
由于本章的全部工作都针对冠状动脉，因此，与第\ref{subsec:coronary_setup_data}节类似，我们将包含完整心脏区域的部分从整个原始数据剥离出来（见图\ref{fig:coronary_enhanced_original}）。
\begin{figure}[t]
\centering
...
\caption[两种像素亮度处理结果对比]{两种像素亮度处理结果对比：(a) 非线性亮度映射（$m = 80$, $n = 120$）；(b) 二值阈值滤波（$\text{lower threshold} = 40$, $\text{upper threshold} = 200$）。}%
