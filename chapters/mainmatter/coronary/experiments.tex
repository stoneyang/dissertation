...
\caption[预处理结果]{预处理结果：(a) 边缘保护前提下的平滑（$\text{传导参数} = 0.9$）；(b) 阈值滤波（$\text{TH}_{\text{lower}} = 160$, $\text{TH}_{\text{upper}} = 600$）。}%
...
\caption[水平集演进过程]{水平集演进过程：(a) 通过快速步进算法生成的初始水平集围线；(b) 由CURVES驱动的围线演进结果；(c) 对演进结果进行二值阈值滤波处理后的结果。}%
\label{fig:Evolution}
\end{figure}
