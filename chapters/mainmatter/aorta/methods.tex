...
因此，式(\ref{aorta_min_model})所表示的最小值可以重新表示为：
\begin{equation}
...
根据式(\ref{aorta_evolution_model})，该演进过程将会在$\frac{\partial \mathcal{C}(t)}{\partial t} = 0$时停止，这意味着目标物体的边缘被成功探知。
假设曲线$\mathcal{C}$是一个函数的水平集$u(\cdot):[0,a] \times [0,b] \rightarrow \mathrm{R}$。
考虑式(\ref{eqn:aorta_min_model_2})和式(\ref{aorta_evolution_model})，之前的测地学计算即可转换为寻找如下方程的稳态解：
\begin{equation}
...
该方程的等号右侧即为式(\ref{eqn:aorta_min_model_2})中的Euler-Lagrange。
