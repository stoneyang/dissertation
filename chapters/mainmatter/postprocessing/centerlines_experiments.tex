...
在本章的实验中，计算机程序由C++实现，并运行于一台搭载Intel 2.83GHz CPU以及4GB内存的台式计算机上。
...
\caption[区段模型及其顶点连接性验证]{区段模型及其顶点连接性验证：(a) 主动脉下半段表面模型（由$205,590$个多边形构成）与VOI提取的模型局部（红色方框内）；(b) 血管表面模型的连接性验证结果（包含的多边形面片的数量：$70,625$）。}
\end{figure}
...
\caption[连接性检验前后的多边形数量（中心线）]{连接性验证前后多边形的数量：(a) 本章实验数据；(b) 完整的血管模型。}
