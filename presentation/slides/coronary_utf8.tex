...
\begin{itemize}
  \item \textbf{特征图像计算}：
  \begin{enumerate}
    \pause \item 基于Gaussian核运算所得到的梯度图像（$\sigma = 0.9$）
    \pause \item 通过S函数实现的像素亮度非线性映射（$m = -80$, $n = 120$）
  \end{enumerate}
\end{itemize}
\begin{columns}[b,onlytextwidth]
\begin{column}{.5\textwidth}
...
\begin{itemize}
  \item \textbf{水平集演进过程}：
  \begin{enumerate}
    \pause \item 通过快速步进算法生成的初始水平集围线
    \pause \item 由CURVES驱动的围线演进结果
    \pause \item 对演进结果进行二值阈值滤波处理后的结果
  \end{enumerate}
\end{itemize}
\begin{columns}[b,onlytextwidth]
