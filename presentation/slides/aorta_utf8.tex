...
\begin{itemize}
  ...
  \begin{enumerate}
    \pause \item 保护物体边缘的平滑处理（曲率各向异性扩散，$\text{传导参数} = 9.0$）
    \pause \item 梯度幅值计算（基于Gaussian核，$\sigma = 0.9$）
    \pause \item 像素亮度非线性映射（S型函数，$m = -3$, $n = 20$）
  \end{enumerate}
\end{itemize}
...
\begin{itemize}
  \item \textbf{水平集演进过程}：
  \begin{enumerate}
    \pause \item 阈值滤波（$\text{TH}_{\text{lower}} = 300\text{HU}$, $\text{TH}_{\text{upper}} = 2000\text{HU}$）
    \pause \item 从某一种子点开始，利用快速步进法进行的初始水平集演进
    \pause \item 利用测地活动轮廓进行边缘检测（迭代次数: $100$）
    \pause \item 利用二值阈值滤波完成像素亮度的反转（明亮区域的亮度值: $255$, 黑暗区域的亮度值: $0$）
  \end{enumerate}
\end{itemize}
\begin{columns}[b,onlytextwidth]
...
\begin{itemize}
  \item \textbf{主动脉内腔分割结果评估}：
  \begin{enumerate}
    \pause \item 基于Jaccard系数（均值为$0.933$，标准差为$0.0240$）
	\begin{itemize}
	\item 交集占并集的比重
	\end{itemize}
    \pause \item 基于Dice系数（均值为$0.965$，标准差为$0.0128$）
	\begin{itemize}
	\item 交集占和平均的比重
	\end{itemize}
  \end{enumerate}
