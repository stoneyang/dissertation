...
\begin{itemize}
\pause \item 走向，半径，分岔位置
\end{itemize}
\pause \item 为虚拟导管行进状态的探测提供基准
\begin{itemize}
\pause \item 状态探测：沿中心线均匀分布的空心球（半径小于血管内径）
\end{itemize}
\end{itemize}
...
\begin{itemize}
  ...
  \begin{itemize}
    \pause \item 主动脉下半段表面模型（由$205,590$个多边形构成）与VOI提取的模型局部（红色方框内）
    \pause \item 所提取区段血管表面模型的连接性验证结果（多边形面片数量：$70,625$）
  \end{itemize}
\end{itemize}
...
\begin{itemize}
  ...
  \begin{itemize}
    \pause \item 基于CTA的表面模型（多边形的数量：$757,538$）
    \pause \item 表面模型的中心线
  \end{itemize}
\end{itemize}
\begin{columns}[b,onlytextwidth]
