...
\node [above right] at (-2.2,0.25) {\tiny \fs \bf 二值阈值滤波};
\node [above right] at (-2.2,3.25) {\tiny \fs \bf 测地活动轮廓};

\node [above right] at (-7.7,7.25) {\tiny \fs \bf 初始水平集演进};

\node [above right] at (2.87,7.25) {\tiny \fs \bf 特征图像计算};

\node [above right] at (-2.15,11.25) {\tiny \fs \bf 二值阈值滤波};
\node [above right] at (-2.75,14.25) {\tiny \fs \bf 曲率各向异性扩散};
\node [above right] at (-1.85,17.25) {\tiny \fs \bf ROI体数据};
